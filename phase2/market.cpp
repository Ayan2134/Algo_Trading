...
struct trade{
    ...
    bool operator<(const trade& other) const {
        if (price == other.price) {
            if (time_arrive == other.time_arrive) {
                return stock < other.stock;
            }
            return time_arrive > other.time_arrive;  //the time at which they arrive less means more priority
        }
    ...
    bool operator==(const trade& other) const {
        return (time_arrive == other.time_arrive) &&
               (price == other.price) &&
               (broker == other.broker);
    }
